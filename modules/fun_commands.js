	...
	bomb:function(global_params,message_type){
		global_params.message.delete(0).then(function(msg){
			...
			if(mentioned_count === 1){
				...
				msg.channel.send("<@"+msg.author.id+">, sorry you can't bomb "+mentioned_count+" users atm.");
			}else if(mentioned_count <= 0 ){
				msg.channel.send("<@"+msg.author.id+">, who should I bomb? You didn't mention anyone.");
			}	
		}).catch(function(err){
			global_params.logger.error(err);
		});
